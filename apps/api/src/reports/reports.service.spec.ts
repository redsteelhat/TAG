...
describe("ReportsService", () => {
  ...
  it("includes maintenance entry cost-per-km reserve in daily profit", async () => {
    const prisma = {
      $transaction: jest.fn((queries: Array<Promise<unknown>>) =>
        Promise.all(queries),
      ),
        ...
        findMany: jest.fn().mockResolvedValue([
          {
            ...
            maintenance_date: new Date("2026-06-01T00:00:00.000Z"),
            title: "Yağ ve filtre",
            vehicle_id: "vehicle_1",
          },
        ]),
    ...
    expect(result.maintenanceReserve).toBe("80.00");
  ...
  it("posts monthly direct fixed costs on the due date", async () => {
    const prisma = {
      $transaction: jest.fn((queries: Array<Promise<unknown>>) =>
        Promise.all(queries),
      ),
      trip: {
        aggregate: jest.fn().mockResolvedValue({
          ...
          _sum: {
            ...
            allocated_other_variable_cost: new Prisma.Decimal("0"),
            allocated_package_cost: new Prisma.Decimal("0"),
            cash_net_profit: new Prisma.Decimal("1200"),
            cancellation_income: new Prisma.Decimal("0"),
            deadhead_km: new Prisma.Decimal("0"),
            duration_minutes: 240,
            estimated_fuel_cost: new Prisma.Decimal("0"),
            gross_income: new Prisma.Decimal("1200"),
            tip_amount: new Prisma.Decimal("0"),
            total_income: new Prisma.Decimal("1200"),
            total_km: new Prisma.Decimal("120"),
            true_net_profit: new Prisma.Decimal("1200"),
            trip_km: new Prisma.Decimal("120"),
          },
        }),
      },
      shift: {
      ...
      recurringExpense: {
        findMany: jest.fn().mockResolvedValue([
          {
            allocation_method: FixedCostAllocationMethod.MONTHLY_DIRECT,
            amount: new Prisma.Decimal("300"),
            expense_type: ExpenseType.FIXED,
            starts_at: new Date("2026-06-01T00:00:00.000Z"),
            ends_at: null,
            next_due_at: new Date("2026-06-18T00:00:00.000Z"),
            period: AllocationType.MONTHLY,
            vehicle_id: "vehicle_1",
          },
        ]),
    ...
    expect(result.fixedExpenses).toBe("300.00");
    expect(result.weeklyFixedCostShare).toBe("300.00");
    expect(result.fixedCostPerKm).toBe("2.50");
    expect(result.netProfit).toBe("900.00");
    expectProfitReportFormula(result);
  });
...
function expectKmProfitabilityFormula(report: {
  ...
  expectMoney(
    report.costPerKm,
    divideOrZero(decimal(report.totalCost), totalKm),
  );
  expectMoney(
