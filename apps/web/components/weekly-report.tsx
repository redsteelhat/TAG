...
export function WeeklyReport() {
  ...
  useEffect(() => {
    ...
    void loadReport(token, weekStart, vehicleId);
  }, []);
  ...
  async function loadReport(
    ...
    try {
      const response = await getJson<WeeklyProfitResponse>('/reports/weekly-profit', {
        accessToken: token,
        query: {
      ...
      setReport(null);
      setMessage(error instanceof Error ? error.message : 'Haftalik rapor alinamadi.');
    } finally {
      setIsLoading(false);
    }
  }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    void loadReport(accessToken, weekStart, vehicleId);
  }
      ...
      {report ? (
        ...
        <section className="empty-state-panel">
          <div>
            ...
            <p>
              Hafta baslangici ve arac filtresini secip haftalik net kar, maliyet
              ve verimlilik metriklerini tek ekranda gorebilirsin.
            </p>
          </div>
        </section>
      )}
...
function getCurrentWeekStartInputValue() {
  const date = new Date(`${getLocalDateInputValue()}T00:00:00`);
